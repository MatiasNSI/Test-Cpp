...
int* climbingLeaderboard(int scores_count, int *scores, int alice_count,
	...
	//Guardo la tabla de scores sin repetir en el set finalResults
	for (int i = 0; i < scores_count - 1; i++) {
		finalResults.insert(scores[i]);
	}

	for (int i = 0; i <= alice_count - 1; i++) {
		int count = 1;
		for (int finalResult : finalResults) {
			if (alice_count[i] < finalResult) {
				count++;
			}
		}
